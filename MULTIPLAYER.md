...
The investigator's online board and public log do not reveal Jack's tentative starting location until he records his first move, including after changes, undo, or refresh. Throughout an unfinished game, investigator display state excludes Jack's hidden position, trail, unreached discoveries, and draft movement type/path. Selection rings and outline sizing also require a Jack-visible view. The investigator's **Public actions** counter excludes private planning actions, so drafts cannot reveal themselves through the counter. Only committed public movement information is shown; destinations remain secret. The finished-game recap still reveals the full history as before. Investigator actions still appear live for Jack. This is a UI privacy boundary, consistent with the existing trust model: both authenticated players still receive the full verified history, not a cryptographically redacted view.

Investigator hover previews reset when the public turn/evidence changes. They require fresh pointer movement, so a stationary cursor does not carry old yes/no colors into the next turn. Private Jack edits do not reset the preview or alter the displayed board. The draft-privacy and hover fixes are frontend-only: publish the website and refresh; they do not require another Worker deployment. Test them locally with `npx playwright test tests/e2e/onlinePrivacy.spec.ts` (also included in `npm run test`).
...
Creating an online game automatically copies the opposite side's invitation link, with a success or fallback notice. Where supported, the clipboard write starts within the Create button's user gesture using promised text, so the server response can arrive asynchronously without losing clipboard activation. If the browser refuses clipboard access, the game still opens and **Copy invitation link** remains available. A failed creation never copies a placeholder or empty string. Joining, refreshing, resuming, and replacement invitations do not automatically overwrite the clipboard. This clipboard behavior is frontend-only and needs no Worker redeployment.
